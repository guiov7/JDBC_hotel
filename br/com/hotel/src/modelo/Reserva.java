...
public class Reserva {
	...
	public static double valor(LocalDate dataEntrada, LocalDate dataSaida) {
		if (dataSaida.isBefore(dataEntrada)) {
			throw new IllegalArgumentException("A data de saída não pode ser anterior de entrada.");
		}
		if (dataSaida.isEqual(dataEntrada)) {
			throw new IllegalArgumentException("Selecione uma data de saída posterior.");
		}
		LocalDate now = LocalDate.now();
		if (dataEntrada.isBefore(now) || dataSaida.isBefore(now)) {
			throw new IllegalArgumentException("Datas de saída e entrada precisam ser posteriores a data atual.");
		}
